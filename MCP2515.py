...
class MCP2515():
    ...
    def WriteBytes(self, address, data):
        self.cs.value(0)
        self.spi.write(bytearray([CAN_WRITE, address, data]))
        self.cs.value(1)
        
    def SetBitModify(self, address, mask, value):
        self.cs.value(0)
        self.spi.write(bytearray([CAN_BIT_MODIFY, address, mask, value]))
        self.cs.value(1)
    ...
    def Send(self, CAN_ID, CAN_TX_Buf, length1):
        ...
        length = len(data)
        self.WriteBytes(TXB0D0 - 1, length & 0x0F)  # DLC

        for i in range(length):
            self.WriteBytes(TXB0D0 + i, data[i])

        self.WriteBytes(TXB0CTRL, 0x08)  # Request transmission

        # Request transmission
        self.WriteBytes(TXRTSCTRL, 0x01)  # Set TXB0RTS
    ...
    def _check_message(self):
        status = self.ReadByte(CANINTF)
        return status & RX0IF

    def _read_single_frame(self):
        id_high = self.ReadByte(RXB0SIDH)
        id_low = self.ReadByte(RXB0SIDL)
        length = self.ReadByte(RXB0D0 - 1) & 0x0F
        data = []
        for i in range(length):
            data.append(self.ReadByte(RXB0D0 + i))
